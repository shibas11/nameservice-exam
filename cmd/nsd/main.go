...
func InitCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command { // generate genesis state from the configuration
	cmd := &cobra.Command{
		Use:   "init",
...
func AddGenesisAccountCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command { // 계정 추가
	cmd := &cobra.Command{
